...
document.addEventListener("keydown", function(e) {
    if (e.key === "Escape" && container.classList.contains("show-nav")) container.classList.remove("show-nav");
});
navItems.addEventListener("click", function(e) {
    if (e.target.classList.contains("nav-item")) container.classList.remove("show-nav");
});
...
window.onload = function() {
    ...
    for(let i = 0; i < splitText.length; i++)text.innerHTML += `<span>${splitText[i]}</span>`;
    let char = 0;
